<h1>Преговор на концепции от УП и добавяне на C++ специфики. Въведение в ООП. Структури, обединения, енумерации и пространства на имената.</h1>

<h3>Потоци за вход и изход</h3>

В C се използват стандартно `scanf` и `printf` за вход и изход респективно.

В C++ техните еквиваленти са потоците за вход и изход `std::cin` и `std::cout`.

```c++
#include <iostream> // Библиотеката, която съдържа std::cin и std::cout.
...
Библиотеката `iostream` също така добавя и `std::endl`, което специфична имплементация на знака за край на реда, в зависимост на коя платформа се пуска приложението.
...
Паметта, която можем да използваме в **C/C++** има 4 основни типа:
<br/>
...
<h3>Преговор на указатели и динамична памет.</h3>

Как се заделя динамична памет в C:
...
int main(int argc, char** argv)
{
	...
	int* ptr = (int*) malloc(1 * sizeof(int)); 	// int in the dynamic memory (we use a pointer on the stack to point to the int)
	// *ptr = 5;								// The program will crash if malloc hasn't given us a valid pointer!!!

	if(ptr == NULL) // We can check with !ptr | ptr == NULL
	{
