...
int main(){
    ...
    int64_t *ptr=(int64_t *)bptr;    // integer pointer which gets the address of vptr which is in first four/eight bytes of Base class object
    ...
    bptr=new Derived;                // Base class pointer points to Derived class object i.e. Up casting hence overriding will take place
                                     ...
                                     // After overriding in VTABLE  at 2 nd index which was holding the address of Base::sun() will not override because of there no definition of gun() in Derived class
